...
#define kSeletImgTopSpace 2 //选中按钮图片 距离标签栏上边的距离
...
- (void)viewWillAppear:(BOOL)animated{
    ...
    _selectIV.frame = CGRectMake(10 + self.selectedIndex * kScreenWidth/self.viewControllers.count, kSeletImgTopSpace, kScreenWidth/self.viewControllers.count-20, 44);
...
- (void)createTabBarButton{
    ...
    //✅选中的背景图片
    _selectIV = [[UIImageView alloc]initWithFrame:CGRectMake(10, kSeletImgTopSpace, kScreenWidth/count-20, 44)];
...
- (instancetype)initWithFrame:(CGRect)frame tabBarItem:(UITabBarItem *)tabBarItem index:(int)index{
    ...
    if (self = [super initWithFrame:frame]) {
        ...
        if (tabBarItem.title) {
            ...
            //如果没有 添加imageView 且图片高度增加
            
            imageView.frame = CGRectMake((frame.size.width-40)/2, 4, 40, 40);
            
            if (index == 2) {
                
                imageView.frame = CGRectMake((frame.size.width-49)/2, 0, 49, 49);
